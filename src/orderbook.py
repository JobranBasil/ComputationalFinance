...
class OrderBook:
    ...
    def _match(self, taker: Order) -> List[Trade]:
        ...
        while taker.qty > 0 and can_cross():
            ...
            print(f"trade is {Trade(ts=taker.ts,price=px,qty=fill,aggressor_side=taker.side,maker_order_id=maker.order_id,taker_order_id=taker.order_id,maker_trader_id = maker.trader_id, taker_trader_id = taker.trader_id)}")
