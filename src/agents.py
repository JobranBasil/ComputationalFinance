...
class BaseAgent:
    ...
    def iceberg_step(self, t: int, book: OrderBook) -> Action:
        # advance iceberg by one action
        if self.iceberg is None:
            return None

        ice = self.iceberg

        # check if active slice still in book
