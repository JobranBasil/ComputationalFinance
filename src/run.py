...
def plot_snapshots(snapshots: list[dict], out_path: str) -> None:
    ...
    tick = snapshots[0]["bids"][0][0] - snapshots[0]["bids"][1][0]
    bar_width = tick * 0.4
    half_width = tick * 22  # controls how many levels are visible — adjust as needed
        ...
        for trade in trades_this_step:
            if mmAS_agent is not None:
                if trade.maker_trader_id == mmAS_agent.trader_id or trade.taker_trader_id == mmAS_agent.trader_id:
                    ...
                    inventory_changes += 1
                    #Clear up slot
                    if trade.maker_order_id == mmAS_agent.last_bid_id or trade.taker_order_id == mmAS_agent.last_bid_id:
                        mmAS_agent.last_bid_id = None
                    elif trade.maker_order_id == mmAS_agent.last_ask_id or trade.taker_order_id == mmAS_agent.last_ask_id:
